...
//        summonerName = summonerName.replaceAll(" ", "%20");
        String[] nameAndTag = riotApiService.splitNameAndTag(summonerName);

        Summoner apiResult = riotApiService.loadUser(nameAndTag[0], nameAndTag[1]);
...
//        summonerName = summonerName.replaceAll(" ", "%20");
        ...
        Summoner apiResult = riotApiService.loadUserWithTag(nameAndTag[0], nameAndTag[1]);
        ...
        LeagueEntryDTO leagueEntrySoloDTO = riotApiService.loadSoloRank(apiResult.getId(), apiResult.getDataId());
        LeagueEntryDTO leagueEntryFlexDTO = riotApiService.loadFlexRank(apiResult.getId(), apiResult.getDataId());
        ...
        //게임 정보 불러오기.
        List<String> gameInfo = riotApiService.loadGameList(apiResult.getPuuid(), 0, 5);
        ...
        if (gameInfo.size() != 0) {
            for (int i = 0; i < gameInfo.size(); i++) {
                ...
                gameInfoData = riotApiService.loadGameInfo(gameInfo.get(i));
                metaDataData = riotApiService.loadMetaDataInfo(gameInfo.get(i));
...
//        }
        ...
        Summoner summoner = riotApiService.loadUserWithPuuid(participants.get(0));
        Summoner summoner1 = riotApiService.loadUserWithPuuid(participants.get(1));
        model.addAttribute("test", summoner);
        model.addAttribute("test1", summoner1);

        GameInfo time = riotApiService.loadGameInfo(gameInfo.get(0));
        Date gameEndTime = riotApiService.convertUnixTimeToUTC(time.getGameEndTimestamp());
        String diffEndTimeNowTimeFormat_1 = riotApiService.diffCurrentTimeAndParam(gameEndTime);
        String diffEndTimeNowTimeFormat_2 = riotApiService.diff(gameEndTime);
